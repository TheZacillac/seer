...
def test_stream_bulk_caps_concurrent_jobs(monkeypatch):
    ...
    async def scenario() -> None:
        ...
        assert not second.done(), (
            "second stream must block on the semaphore, not start immediately"
        )

        # Releasing the first job frees the slot (via the future's
        # done-callback), which unblocks the second.
        release.set()
