# Diff Output Rework Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace `HumanFormatter::format_diff` with an aligned two-column table featuring domain column headers, `=`/`≠` match markers colored green/red, per-column wrapping, and one-per-line multi-value fields.

**Architecture:** Build a small intermediate representation (`DiffRow` / `DiffSection`) that flattens `DomainDiff` into uniform rows. Pure helper functions compute equality, wrap long values, and size columns. A single render function walks the intermediate model and emits the final string. All changes stay in `seer-core/src/output/human.rs`; no data-model changes, no touches to JSON/Markdown formatters.

**Tech Stack:** Rust 2021 edition, `colored` crate (via `CatppuccinExt` trait), `chars().count()` for display-width (ASCII assumed; non-ASCII safe-fallback). Tests use the existing `#[cfg(test)] mod tests` block with `HumanFormatter::new().without_colors()`.

**Spec:** `docs/superpowers/specs/2026-04-17-diff-output-rework-design.md`
...
- **Modify:** `seer-core/src/output/human.rs`
  - Replace `format_diff` method body (currently lines 2120–2235 in the `impl OutputFormatter for HumanFormatter` block).
  - Add new private free functions at the bottom of the file (after the `impl` block, before `#[cfg(test)]`): `eq_opt_str_trimmed`, `eq_as_set`, `wrap_cell`, `build_diff_sections`, `compute_column_width`, plus two private types `DiffRow` and `DiffSection`.
  - Add new tests in the existing `mod tests` block at the bottom of the file.

No other files are touched.

---

## Task 1: Equality helpers

**Files:**
- Modify: `seer-core/src/output/human.rs` (add free functions near end of file, above `#[cfg(test)]`)
- Test: `seer-core/src/output/human.rs` (same `mod tests` block)

- [ ] **Step 1: Write the failing tests**

Append to `mod tests { ... }` in `seer-core/src/output/human.rs`:
...
Run: `cd seer-core && cargo test --lib output::human::tests::eq_ 2>&1 | tail -20`
Expected: compile error — `cannot find function eq_opt_str_trimmed` and `eq_as_set`.

- [ ] **Step 3: Implement the helpers**

Insert at end of `seer-core/src/output/human.rs`, immediately before `#[cfg(test)]`:
...
cd seer-core
cargo fmt
git add src/output/human.rs
git commit -m "feat(output): add equality helpers for diff row matching

eq_opt_str_trimmed normalizes Option<String> with whitespace trim and
empty-string-as-None. eq_as_set compares list fields as sorted sets so
nameserver order does not cause a false mismatch."
```

---

## Task 2: Cell wrapping

**Files:**
- Modify: `seer-core/src/output/human.rs`
- Test: `seer-core/src/output/human.rs` (same `mod tests`)
...
Run: `cd seer-core && cargo test --lib output::human::tests::wrap_cell 2>&1 | tail -20`
Expected: compile error — `cannot find function wrap_cell`.

- [ ] **Step 3: Implement `wrap_cell`**
...
cargo fmt
git add src/output/human.rs
git commit -m "feat(output): add wrap_cell helper for diff column wrapping

wrap_cell splits long cell content at whitespace boundaries when
available and hard-breaks otherwise, capped at max_width chars."
```

---

## Task 3: Intermediate model + builder
...
fn build_diff_sections_preserves_field_order() {
    ...
    assert_eq!(
        dns_labels,
        vec!["Resolves", "A Records", "Nameservers"]
    );
...
Run: `cd seer-core && cargo test --lib output::human::tests::build_diff_sections 2>&1 | tail -20`
Expected: compile error — types `DiffRow`, `DiffSection` and function `build_diff_sections` not found.
...
fn build_diff_sections(diff: &crate::diff::DomainDiff) -> Vec<DiffSection> {
    ...
    let dns_section = DiffSection {
        ...
        rows: vec![
            DiffRow {
                label: "Resolves",
                a_values: vec![if dns.resolves.0 { "yes".into() } else { "no".into() }],
                b_values: vec![if dns.resolves.1 { "yes".into() } else { "no".into() }],
...
cargo fmt
git add src/output/human.rs
git commit -m "feat(output): add DiffRow/DiffSection model and builder

build_diff_sections flattens DomainDiff into a uniform Vec<DiffSection>
with per-row match flags, em-dash placeholders, and one-item-per-row
multi-value lists — decoupling data layout from rendering."
```

---

## Task 4: Column width computation

**Files:**
- Modify: `seer-core/src/output/human.rs`
- Test: `seer-core/src/output/human.rs`
...
Run: `cd seer-core && cargo test --lib output::human::tests::compute_column_width 2>&1 | tail -20`
Expected: compile error — `cannot find function compute_column_width`.

- [ ] **Step 3: Implement `compute_column_width`**

Add below `build_diff_sections`:
...
fn compute_column_width(sections: &[DiffSection], domain_a: &str, domain_b: &str) -> usize {
    let mut widest = domain_a
        .chars()
        .count()
        .max(domain_b.chars().count());
    ...
    widest.min(DIFF_COLUMN_CAP).max(1)
}
```
...
cargo fmt
git add src/output/human.rs
git commit -m "feat(output): add compute_column_width for diff table

Widest-across-all-rows-and-domains value, capped at 40, minimum 1."
```

---

## Task 5: Replace `format_diff` with the new renderer

**Files:**
- Modify: `seer-core/src/output/human.rs` (replace lines 2120–2235, currently the entire `fn format_diff` body in the `impl OutputFormatter`)
- Test: `seer-core/src/output/human.rs`
...
fn format_diff_shows_column_headers_with_domain_names() {
    ...
    assert!(out.contains("example.com"), "missing domain_a in output:\n{}", out);
    assert!(out.contains("google.com"), "missing domain_b in output:\n{}", out);
...
fn format_diff_all_matching_has_no_neq() {
    ...
    assert!(!out.contains('≠'), "all-match diff should have no ≠:\n{}", out);
}
```

- [ ] **Step 2: Run tests to verify they fail**

Run: `cd seer-core && cargo test --lib output::human::tests::format_diff 2>&1 | tail -30`
Expected: tests compile, but some fail against the current format — the assertions on `≠`, `─`, em-dash, and one-per-line multi-values will not hold.

- [ ] **Step 3: Replace the body of `format_diff`**

Locate `fn format_diff(&self, diff: &crate::diff::DomainDiff) -> String` (currently around lines 2120–2235). Replace its entire body with:
...
Also add the `pad_right` helper near the other free functions (below `compute_column_width`):
...
Run: `cd seer-core && cargo test --lib output::human::tests::format_diff 2>&1 | tail -40`
Expected: all `format_diff_*` tests pass.

Also run the full diff-related suite to catch regressions:
Run: `cd seer-core && cargo test --lib output::human 2>&1 | tail -20`
Expected: all tests pass.

- [ ] **Step 5: Sanity-check the rendered output**

Run the CLI against a real example and visually confirm the new layout:

```bash
cd /home/zac/Projects/arcanum_suite/seer
cargo run --quiet -p seer-cli -- diff example.com google.com
```

Expected: output shows column headers, aligned `=`/`≠` markers in green/red, multi-value nameservers each on their own line, em-dashes for missing values. Capture the output for the commit message.
...
git commit -m "feat(output): rework diff output into aligned two-column table

Replaces HumanFormatter::format_diff with a table layout: domain names
as column headers, =/≠ markers per row colored green/red, per-column
wrapping at 40 chars, one-per-line multi-value fields (A records,
nameservers), and em-dash placeholders for missing values. Set-based
equality for nameservers/A records so reordered lists match."
```

---

## Task 6: Whole-plan regression check

**Files:** none modified — this is a verification-only task.

- [ ] **Step 1: Run the full Rust test suite**

Run: `cd /home/zac/Projects/arcanum_suite/seer && cargo test 2>&1 | tail -30`
Expected: all tests pass. No panics, no regressions in other `output::` tests, `diff::` tests, or elsewhere.

- [ ] **Step 2: Run clippy**

Run: `cd /home/zac/Projects/arcanum_suite/seer && cargo clippy -- -D warnings 2>&1 | tail -30`
Expected: no warnings.

- [ ] **Step 3: Run fmt check**

Run: `cd /home/zac/Projects/arcanum_suite/seer && cargo fmt --check`
Expected: exit 0 with no output. If it exits non-zero, run `cargo fmt` and amend the relevant commit with the formatting fix (or commit the fix as a follow-up).

- [ ] **Step 4: Run the CLI on a domain pair that exercises every field**

```bash
cd /home/zac/Projects/arcanum_suite/seer
cargo run --quiet -p seer-cli -- diff github.com gitlab.com
```

Expected: aligned table, clear match/differ markers, no crashes, no obvious misalignments. Also try a pair where one domain is unregistered, to exercise the em-dash paths:

```bash
cargo run --quiet -p seer-cli -- diff github.com this-domain-does-not-exist-123456.com
```

Expected: the missing-data side renders em-dashes; no panic.
...
- **Spec coverage:** layout (Task 5), column widths (Task 4), wrapping (Task 2), multi-value one-per-line (Task 3+5), equality rules (Tasks 1 & 3), colors (Task 5, via `self.success`/`self.error`), em-dash placeholder (Task 3), plain mode (tested in Task 5), all-matching edge case (tested in Task 5), non-goals respected (only `human.rs` touched, no `DomainDiff` changes, no JSON/Markdown changes).
- **Placeholder scan:** every step has concrete code or an exact command. No TBDs.
- **Type consistency:** `DiffRow` / `DiffSection` defined in Task 3, consumed by the same names in Tasks 4 and 5. `build_diff_sections`, `compute_column_width`, `wrap_cell`, `pad_right`, `eq_opt_str_trimmed`, `eq_as_set` are referenced consistently across tasks.
