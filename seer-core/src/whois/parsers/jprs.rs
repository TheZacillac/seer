...
/// Parses .jp domains using the JPRS format.
pub(super) fn parse(domain: &str, server: &str, raw: &str) -> WhoisResponse {
    ...
    let first_date = |pattern: &Regex| {
        pattern
            ...
            .find_map(|m| parse_jprs_date(m.as_str()))
    };
    ...
    let expiration_date = first_date(&EXPIRES_PATTERN).or_else(|| {
        status.iter().find_map(|s| {
            STATE_EXPIRY_PATTERN
                ...
                .and_then(|m| parse_jprs_date(m.as_str()))
        })
    });
...
[最終更新]                      2024/04/01 01:05:10 (JST)"#;
    ...
    fn test_jprs_nameservers() {
        let result = parse("tokyo.jp", "whois.jprs.jp", SAMPLE_JPRS_RESPONSE);
    ...
    fn test_jprs_domain_with_dates() {
        let result = parse("example.jp", "whois.jprs.jp", SAMPLE_JPRS_DOMAIN);
...
[State]                         Active"#;
    ...
    fn test_jprs_en_fallback_strips_bracket_label() {
        let result = parse("example.jp", "whois.jprs.jp", SAMPLE_JPRS_EN_BRACKET_NS);
...
[Last Updated]                  2026/03/01 01:05:03 (JST)"#;
    ...
    fn test_jprs_current_format_nameservers_and_dates() {
        let result = parse("jprs.jp", "whois.jprs.jp", SAMPLE_JPRS_CURRENT);
    ...
    fn test_jprs_no_match_is_available() {
        let raw = "[ JPRS database provides information on network administration. Its use is    ]\n\
                   [ restricted to network administration purposes. For further information,     ]\n\
                   No match!!\n";
        let result = parse("nosuchdomain.jp", "whois.jprs.jp", raw);
        assert!(
    ...
    fn test_jprs_current_format_english_labels() {
        let result = parse("jprs.jp", "whois.jprs.jp", SAMPLE_JPRS_CURRENT_EN);
    ...
    fn test_jprs_empty_fields_do_not_capture_next_line() {
        ...
        let result = parse("example.co.jp", "whois.jprs.jp", raw);
        assert!(
...
";
    ...
    fn test_jprs_attribute_type_co_jp_labels() {
        ...
        let result = parse("google.co.jp", "whois.jprs.jp", SAMPLE_JPRS_CO_JP_EN);
        ...
        let result = parse("google.co.jp", "whois.jprs.jp", &signed);
        assert_eq!(result.dnssec.as_deref(), Some("signedDelegation"));
    }
    ...
    fn test_jprs_date_at_chrono_min_does_not_panic() {
        ...
        assert_eq!(parse_jprs_date(&format!("{min} (JST)")), None);
        ...
        let result = parse("example.jp", "whois.jprs.jp", &raw);
